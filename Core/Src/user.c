 ...
 */
volatile float motor_rpm_filtered = 0;

/*
 * 兼容旧变量名。
 *
 * 当前 motor_rpm 同步为 motor_rpm_filtered。
 * 这样 pid_pwm_update() 中原来使用 motor_rpm 的逻辑不用大改。
 */
 ...
 * 霍尔捕获异常跳变调试变量。
 *
 * 用于观察 hall_capture_value 是否出现异常尖峰。
 */
...
void Register_Map_Apply(void)
{
	...
	/*
	 * Reset PID when entering or leaving PID mode.
 ...
 *
 * 作用：
 *   减小霍尔单边沿瞬时测速造成的跳动，使 CubeMonitor 显示更稳定，
 *   也为后续 PID 闭环提供更平滑的反馈值。
 */
void RPM_Filter_Update(float raw_rpm)
{
    if (raw_rpm <= 0.0f)
 ...
 *
 * 注意：
 * 该判断只是粗略诊断。PWM 快速变化、真实转速快速变化时，
 * 也可能触发 spike_count 增加。
 */
void Hall_Capture_Spike_Check(uint32_t capture_value)
{
